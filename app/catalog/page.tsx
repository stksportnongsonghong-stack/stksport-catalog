...
export default function CatalogPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a' }}>
      {toast && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, background: toast.type === 'ok' ? '#0c2210' : '#220c0c', border: `1px solid ${toast.type === 'ok' ? '#266626' : '#0055cc'}`, color: toast.type === 'ok' ? '#6fdf6f' : '#7aadff', padding: '11px 18px', borderRadius: 7, fontSize: 13, fontWeight: 500, boxShadow: '0 4px 24px rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {!showCustMgr && (
        <>
          ...
          {adminUser && (
            <div style={{ background: 'rgba(0,85,204,0.07)', borderBottom: '1px solid rgba(0,85,204,0.18)', padding: '9px 20px' }}>
              <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                <a href={`/export?admin=${encodeURIComponent(adminUser || '')}`} style={{ background: 'transparent', color: '#f5f5f5', border: '1px solid rgba(255,255,255,0.22)', padding: '5px 12px', borderRadius: 5, fontSize: 12, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 5, transition: 'all .18s' }}
                  onMouseOver={e => { (e.currentTarget as HTMLAnchorElement).style.borderColor='#0055cc'; (e.currentTarget as HTMLAnchorElement).style.color='#0055cc' }}
...
function LoadingScreen() {
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
      <div style={{ width: 50, height: 50, background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 24, color: '#fff' }}>S</div>
      ...
      <div style={{ width: 160, height: 3, background: '#1c1c1c', borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ height: '100%', background: '#0055cc', animation: 'loading 1.2s ease-in-out infinite', borderRadius: 2 }} />
      </div>
    </div>
  )
}
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: 20 }}>
        {banners.length > 0 ? (
            ...
            {banners.length > 1 && (
              <>
                ...
                <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 6 }}>
                  {banners.map((_, i) => <div key={i} onClick={() => setCur(i)} style={{ width: i === idx ? 22 : 7, height: 7, borderRadius: 4, background: i === idx ? '#0055cc' : 'rgba(255,255,255,0.35)', cursor: 'pointer', transition: 'all .3s' }} />)}
                </div>
              </>
            )}
            {isAdmin && (
          ...
          <div className={`drag-zone${ov ? ' ov' : ''}`} style={{ minHeight: 120, aspectRatio: '16/5', maxHeight: 400, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}
            ...
            <div style={{ color: '#0055cc', fontWeight: 700, fontSize: 14 }}>ลาก-วางรูป Banner หรือคลิกเลือกไฟล์</div>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isDragOver ? '2px dashed #0055cc' : undefined,
    ...
    >
      ...
      <div style={{ padding: '13px 14px 12px' }}>
        {shirt.category === 'fabric' ? (
          <>
            <div style={{ background: '#0055cc', borderRadius: 6, padding: '5px 10px', marginBottom: 8, display: 'inline-block' }}>
      ...
      <div className="modal-box">
        ...
        {imgPreview ? (
          ...
          <div className={`drag-zone${ov ? ' ov' : ''}`} style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}
            ...
            <div style={{ color: '#0055cc', fontWeight: 700, fontSize: 13 }}>ลาก-วางรูปภาพ หรือคลิกเลือก</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>JPG · PNG · WEBP — อัปโหลดสู่ Supabase Storage</div>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <div style={{ maxHeight: 260, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 5 }}>
        {promotions.map((p) => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#1a1a1a', padding: '8px 12px', borderRadius: 5, border: p.is_active ? '1px solid #0055cc' : '1px solid transparent' }}>
            <div style={{ flex: 1 }}>
            ...
            <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, background: p.is_active ? '#0055cc' : '#333', color: '#fff', cursor: 'pointer' }} onClick={() => toggleActive(p)}>{p.is_active ? 'เปิด' : 'ปิด'}</span>
        ...
        : <div style={{ display: 'grid', gap: 10 }}>
          {list.map((c) => (
            <div key={c.id} style={{ background: '#161616', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg,#0055cc,#003a99)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15, color: '#fff', flexShrink: 0 }}>{(c.name || '?')[0].toUpperCase()}</div>
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, padding: '36px 32px', width: '100%', maxWidth: 400 }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <div style={{ width: 50, height: 50, background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 24, color: '#fff', margin: '0 auto 14px' }}>S</div>
          {badge && <div style={{ display: 'inline-block', background: '#0055cc', fontSize: 9, padding: '2px 10px', borderRadius: 3, fontWeight: 700, letterSpacing: 2, color: '#fff', marginBottom: 10 }}>{badge}</div>}
  ...
  return (
    <div className="modal-bg" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 460, padding: 0, overflow: 'hidden' }}>
        {/* Header */}
        <div style={{ background: 'linear-gradient(135deg,#0055cc,#003a99)', padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {contact.phone1 && (
                    <a href={`tel:${contact.phone1}`} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 10, background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.08)', textDecoration: 'none', transition: 'border-color .15s' }}
                      onMouseOver={e => (e.currentTarget.style.borderColor = '#0055cc')}
    ...
    <div className="modal-bg" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 440, maxHeight: '92vh', padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowY: 'auto', maxHeight: '92vh' }}>
          ...
          {!calculated && (
            <>
              <div style={{ background: 'linear-gradient(135deg,#0055cc,#003a99)', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
              ...
              <div style={{ padding: '16px 20px', display: 'grid', gap: 14 }}>
                ...
                {/* โปรโมชั่น */}
                {activePromo && (
                  <div style={{ background: 'rgba(0,85,204,0.1)', border: '1px solid rgba(0,85,204,0.3)', borderRadius: 8, padding: '10px 14px' }}>
          ...
          {calculated && (
            <div style={{ padding: '16px 20px' }}>
              {/* ปุ่มปิด */}
              <div style={{ background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: '8px 8px 0 0', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 0 }}>
              ...
              <div style={{ background: '#161616', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '0 0 10px 10px', padding: '14px 16px', display: 'grid', gap: 8 }}>
                ...
                {promoChoice === 'free' && bonusQty > 0 && (
                  <div style={{ background: 'rgba(0,85,204,0.1)', borderRadius: 6, padding: '6px 10px', fontSize: 12, color: '#6699ff' }}>
      ...
      <div className="modal-box" style={{ maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: 14 }}>
            ...
            {/* Phone */}
            <div style={{ background: '#0055cc', borderRadius: 10, padding: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#fff', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><circle cx="11" cy="11" r="11" fill="white"/><path d="M14.5 13.5l-1.3 1.3c-2-.5-3.7-2.1-4.2-4.1L10.3 9.4a.5.5 0 000-.7L8.6 7a.5.5 0 00-.7 0L6.5 8.4C6.2 11.7 9.2 15 12.6 14.5l1.3-1.3a.5.5 0 000-.7z" fill="#0055cc"/></svg>
                เบอร์โทรศัพท์
              </div>
              <div style={{ display: 'grid', gap: 8 }}>
                <div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '36px 28px', width: '100%', maxWidth: 380, textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.8)' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          {shopSettings.logo_url ? (
            <img src={shopSettings.logo_url} alt="logo"
              style={{ width: 90, height: 90, borderRadius: '50%', objectFit: 'cover', border: '3px solid rgba(0,85,204,0.5)', margin: '0 auto' }} />
          ) : (
            <div style={{ width: 90, height: 90, background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 36, color: '#fff', margin: '0 auto' }}>S</div>
          )}
        </div>
        {/* Name */}
        ...
        <button onClick={onBrowse}
          style={{ width: '100%', background: '#0055cc', color: '#fff', border: 'none', padding: '14px', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: 15, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
          👕 เข้าชมแบบเสื้อ
        </button>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              {logoUrl ? (
                <img src={logoUrl} alt="logo" style={{ width: 72, height: 72, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(0,85,204,0.4)', flexShrink: 0 }} />
              ) : (
                <div style={{ width: 72, height: 72, background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 28, color: '#fff', flexShrink: 0 }}>S</div>
              )}
              <div style={{ flex: 1 }}>
          ...
          <div style={{ background: '#0d0d0d', borderRadius: 10, padding: 16, textAlign: 'center', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {logoUrl ? (
              ...
              <div style={{ width: 60, height: 60, background: 'linear-gradient(135deg,#0055cc,#003a99)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 24, color: '#fff', margin: '0 auto 8px' }}>S</div>
            )}
